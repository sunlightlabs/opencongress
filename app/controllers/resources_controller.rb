class ResourcesController < ApplicationController
  ...
  def email_friend_send
    ...
    if object_type == 'Bill'
      subject = "OpenCongress: #{item.best_title(:prefixed)}"
    ...
    begin
      Emailer::friend(dest_emails, params[:email][:email], subject, url, item_desc, params[:email][:message]).deliver
  ...
  def write_rep_form
    ...
    if @bill
      @write_rep_email.subject = "RE: #{@bill.best_title(:prefixed)[0..250]}"
    end
