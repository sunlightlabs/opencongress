class WatchdogsController < ApplicationController
  ...
  def index
    ...
    @item_comments = Comment.find_by_sql(["SELECT n1.* FROM comments AS n1
                                            LEFT OUTER JOIN comments as n2
                                              ON (n1.user_id = n2.user_id
                                                AND (n1.created_at < n2.created_at
                                                     OR (n1.created_at = n2.created_at
                                                         AND n1.id < n2.id)))
                                            WHERE n1.user_id in (?)
                                            GROUP BY n1.id, n1.commentable_id, n1.commentable_type, n1.comment, n1.user_id,
                                                     n1.name, n1.email, n1.homepage, n1.created_at, n1.parent_id, n1.title,
                                                     n1.updated_at, n1.average_rating, n1.censored, n1.ok, n1.rgt, n1.lft,
                                                     n1.root_id, n1.fti_names, n1.flagged, n1.ip_address
                                            HAVING COUNT(*) < 3
                                            ORDER BY n1.created_at desc
                                            LIMIT 30;", @watchdog_ids])
