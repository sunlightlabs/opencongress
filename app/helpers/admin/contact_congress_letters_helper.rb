module Admin::ContactCongressLettersHelper
  ...
  def success_percentage(person)
    ...
    succes_cnt = person.formageddon_threads.includes(:formageddon_letters).where("formageddon_threads.created_at >= ? and formageddon_letters.status = 'SENT'", person.formageddon_contact_steps.last.updated_at).size rescue 0
