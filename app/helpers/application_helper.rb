...
EOT
  ...
  def toggler(div_name, show_link_text, hide_link_text, show_link_class = "", hide_link_class = "")
    out = %Q{<span class="" id="show_#{div_name}">} + link_to(show_link_text, "Element.show('hide_#{div_name}');Element.hide('show_#{div_name}');new Effect.BlindDown('#{div_name}', {duration: 0.25});", :class => show_link_class) + "</span>"
    out += %Q{<span class="" id="hide_#{div_name}" style="display:none;">} + link_to(hide_link_text, "Element.show('show_#{div_name}');Element.hide('hide_#{div_name}');new Effect.BlindUp('#{div_name}', {duration: 0.25});", :class => hide_link_class) + "</span>"
  ...
  def ajax_toggler(div_name, show_link_text, hide_link_text, field_two, show_link_class = "", hide_link_class = "")
	...
	def id_class
		"id=\"#{@admin_styles ? 'index' : controller.controller_name}\" class=\"#{params[:person_type]? params[:person_type] : controller.action_name} #{logged_in? ? 'logged-in' : 'logged-out'}\""
	end
	def class_class
