# == Schema Information
#
# Table name: political_notebooks
#
...
class PoliticalNotebook < ActiveRecord::Base
  ...
  has_many :notebook_items, -> { order('created_at DESC') }, 
           :dependent => :destroy #sti, all
  ...
  def can_view?(viewer)
    if self.group.nil?
      return self.user.can_view(:political_notebook,viewer)
    else
