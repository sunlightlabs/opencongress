...
xml.feed "xmlns" => "http://www.w3.org/2005/Atom" do
  ...
  xml.title   "Open Congress : " + @user.login.possessive + " actions"
    ...
    xml.entry do
      ...
      when 'Bookmark'
        ...
        when 'Person'
          ...
          xml.updated i.created_at.strftime("%Y-%m-%dT%H:%M:%SZ")
      ...
      when 'Comment'
        ...
        when 'BillVote'
          ...
          xml.link    "rel" => "alternate", "href" => bill_url(i.bill)
        ...
        when 'Friend'
          ...
          xml.updated i.created_at.strftime("%Y-%m-%dT%H:%M:%SZ")
          xml.content "type" => "html" do
            xml.text! "View #{link_to(i.friend.login.possessive + " Profile", user_profile_url(i.friend.login))}"
          end
        when 'ContactCongressLetter'
          xml.title   "Wrote a letter to Congress titled '#{i.formageddon_threads.first.formageddon_letters.first.subject}'"
          xml.link    "rel" => "alternate", "href" => contact_congress_letter_url(i)
          xml.updated i.created_at.strftime("%Y-%m-%dT%H:%M:%SZ")
          xml.content "type" => "html" do
            xml.text! i.formageddon_threads.first.formageddon_letters.first.message
          end
      end
    end
  end
