...
congress = ENV['CONGRESS'] || Settings.default_congress.to_s
PATH = Settings.govtrack_data_path + "/#{congress}/bills"
...
def get_text_word_count(congress, bill_type, bill_number, text_version)
  ...
  begin
    ...
    text_filename = "#{Settings.govtrack_billtext_path}/#{congress}/#{bill_type}/#{bill_type}#{bill_number}#{text_version}.txt"
    text_file = File.open(text_filename)
  rescue
    begin
      # if that didn't work just use the symlink
      text_filename = "#{Settings.govtrack_billtext_path}/#{congress}/#{bill_type}/#{bill_type}#{bill_number}.txt"
      text_file = File.open(text_filename)
    rescue
      return 0
    end
...
#    outfile = File.new("#{Settings.oc_billtext_path}/#{congress}/#{bill.bill_type}/#{bill.bill_type}#{bill.number}#{text_version}.gen.html", "w+")

    # TODO: Import guard to make sure we don't mess stuff up when default_congress is over
    outfilename = "#{Settings.oc_billtext_path}/#{bill.session}/#{bill_abbrev}/#{bill_abbrev}#{bill.number}#{text_version}.gen.html-oc"
...
if ENV['PARSE_ONLY'].blank?
  Bill.get_types_ordered_new.keys.each do |bill_type|
    ...
    type_bills = Bill.find(:all, :conditions => ["bill_type = ? AND session = ?", bill_type, congress])
    type_bills.each_with_index do |bill, i|
      ...
      if Dir.exists?("#{Settings.govtrack_billtext_diff_path}/#{congress}/#{bill_type_old}")
        bill_version_files = Dir.new("#{Settings.govtrack_billtext_diff_path}/#{congress}/#{bill_type_old}").entries.select { |f| f.match(/#{bill_type_old}#{bill.number}_(.*)\.xml$/) }
      else
      ...
      if bill_version_files and bill_version_files.size > 0
        ...
        while index < version_array.size
          version_file = "#{Settings.govtrack_billtext_diff_path}/#{congress}/#{bill_type_old}/#{bill_type_old}#{bill.number}_#{previous_version}-#{version}.xml"
        ...
        version_file = "#{Settings.govtrack_billtext_path}/#{congress}/#{bill_type_old}/#{bill_type_old}#{bill.number}#{version_array.last[0]}.gen.html"

        parse_from_file(bill, version_array.last[0], version_file)
      else
        bill_files = Dir.new("#{Settings.govtrack_billtext_path}/#{congress}/#{bill_type_old}").entries.select { |f| f.match(/#{bill_type_old}#{bill.number}[a-z]+[0-9]?\.gen\.html$/) }

        bill_files.each do |f|
          md = /([hs][jcr]?)(\d+)(\w+)\.gen\.html$/.match(f)
          bill_type, bill_number, text_version = md.captures

          parse_from_file(bill, text_version, "#{Settings.govtrack_billtext_path}/#{congress}/#{bill_type}/#{f}")
        end
      end
    end
  end
