...
module MaxmindGeoIP
  ...
  def import
    ...
    if dbpath
      ...
      SQLite3::Database.new(dbpath) do |db|
        # The behavior here is to kill all entries because there is no guarantee that range bounds will be consistent
        # from load to load, leaving us with no practical primary key. The number of ranges may not even hold constant.
        GeoIp.destroy_all
        redos = 0
        db.execute("select locs.locId, locs.country, locs.region, locs.city, locs.postalCode, locs.latitude, locs.longitude, ranges.startIpNum as start_ip, ranges.endIpNum as end_ip
          ...
          begin
            ...
            if redos < 3
              ...
              OCLogger.log("#{row[3]}, #{row[2]}, #{row[1]} (#{row[5]}, #{row[6]}) timed out after 2 retries. Moving on...")
            end
          end
  ...
  def extract(path)
    ...
    # This is probably really brittle; depends on specific output from `unzip`, but I've read that the zip library
    # sucks at everything, so let's try this first.
  ...
  def load(rangespath, locspath)
    ...
    # Source data is latin-1, because, y'know, we should put multibyte chars in CSV.
