      ...
      if leg_hash['+current_term'].nil?
        ...
        if leg_hash['+current_term']['state']
          ...
          unless state_name.nil?
            state = State.where(abbreviation: state_abbrev, name: state_name).first_or_create
            if leg_hash['+current_term']['district']
              district = state.districts.where(district_number: leg_hash['+current_term']['district']).first_or_create
            end
          end
        end
      end
